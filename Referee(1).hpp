...
class Referee : public LibXR::Application {
 public:
  ...
  struct [[gnu::packed]] SentryPack {
    ...
    RFID rfid;
    uint32_t current_state; /*哨兵当前姿态*/
    uint32_t comfirm_mech;  /* 己方能量机关是否能进入正在激活状态 */
  };
  ...
  void Publish() {
    ...
    this->sp_.rfid = this->data_.rfid;
    this->sp_.current_state = this->data_.sentry_dec_data.current_state;
    this->sp_.comfirm_mech = this->data_.sentry_dec_data.comfirm_mech;
